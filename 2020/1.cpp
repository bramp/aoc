...
int main() {
    ...
    int a = find_pair(target);
    if (a <= 0) {
        cout << "Answer 1.1: No solution" << endl; 
    } else {
        cout << "Answer 1.1: " << ((target - a) * a) << endl; 
    }

    for (const int a : numbers) {
        // Find a pair that makes (target - a)
        int b = find_pair(target - a);
        if (b <= 0) {
            continue;
        }

        // Now do we have a number
        int c = target - a - b;
        cout << "Answer 1.2: " << (a * b * c) << endl;
        return 0;
    }
    
    cout << "Answer 1.2: No solution" << endl;
}